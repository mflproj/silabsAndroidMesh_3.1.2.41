...
class ExperimentExecutor(
    ...
    private val taskExecutorRetry = Executors.newSingleThreadScheduledExecutor()
    ...
    private fun controlUnicastWithAck() {
        ...
        if (experiments[currentExperiment.ordinal].meshNode != null) {
            if (experiments[currentExperiment.ordinal].isConfigFinished) {
                ...
                repeatTask(Runnable { sendUnicastOnOffSet() })
            } else {
    ...
    private fun factoryResetSubnet() {
        ...
        meshNetworkManager.removeSubnet(subnet, object : MeshNetworkManager.DeleteNetworksCallback {
            ...
            override fun error(subnet: Subnet?, error: ErrorType?) {
                ...
                if (retryTask(Runnable { factoryResetSubnet() })) {
                    return
                }
    ...
    private fun controlGroup() {
        ...
        if (onOffExist) {
            repeatTask(Runnable { sendMulticastOnOffSet(level > 0) })
        }
    }
    ...
    private fun sendMulticastOnOffSet(on: Boolean) {
        ...
        var meshTask: MeshTask? = null
        meshTask = ControlGroup(group).setStatus(generic, experimentParams.requestParams,
                object : GenericGroupHandler<GenericOnOff?> {
                    ...
                    override fun error(group: Group?, error: ErrorType?) {
                        if(error?.type == ErrorType.TYPE.TASK_CANCELED){
                            return
                        }
    ...
    private fun handleUnicastControlResponseForNoAck() {
        if (experimentParams.countElementSetResponse == 1) {
            ...
            if (group != null) {
                handler.postDelayed({
                    taskList.add(sendUnicastGetTask(experiments[currentExperiment.ordinal].meshNode!!.node, group))
                    startTasks()
                }, 2000)
            } else {
                experiments[currentExperiment.ordinal].areResponsesCorrect = false
    ...
    private fun repeatTask(task: Runnable) {
        // repeat to make sure that packet is delivered to the node
        repeat(3){
            handler.postDelayed({
                task.run()
            }, 50)
    ...
    private fun retryTask(task: Runnable? = currentTask!!): Boolean {
        var retryCount = experiments[currentExperiment.ordinal].retryCount
        if (++retryCount <= 3 && task != null) {
            Log.d(TAG, "retry task: retryCount=$retryCount")
            taskExecutorRetry.execute(task)
            return true
        }
        return false
    }
    ...
    companion object {
        ...
        private val bluetoothScanDates = mutableListOf<Date>()

        /**
         * Prevents too often scans that result in [ErrorType.TYPE.COULD_NOT_CONNECT_TO_DEVICE] error.
         * Should be used before every Bluetooth scan.
         */
        fun prepareForBluetoothScan() {
            waitForSafeBluetoothScan()
            saveBluetoothScanDate()
        }

        private fun waitForSafeBluetoothScan() {
            bluetoothScanDates.takeIf { it.size == 5 }
                    ?.let { 35_000 - Date().time + it.removeAt(0).time }
                    ?.takeIf { it > 0 }
                    ?.let {
                        Thread.sleep(it)
                    }
        ...
        private fun saveBluetoothScanDate() {
            val currentTime = Date()
            bluetoothScanDates.add(currentTime)
        }
    }
}
