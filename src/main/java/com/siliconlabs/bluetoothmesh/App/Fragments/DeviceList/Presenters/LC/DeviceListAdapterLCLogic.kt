...
class DeviceListAdapterLCLogic(val listener: DeviceListAdapterLogic.DeviceListAdapterLogicListener) {
    ...
    fun setLCOnOff(meshNode: MeshNode, enable: Boolean) {
        ...
        meshElementControl?.setLCOnOff(mode, transactionId.next(), null, null, acknowledged,
                object : MeshElementControl.LightControlLightOnOffStatusCallback {
                    override fun success(status: LightControlLightOnOffStatus?) {
    ...
    fun refreshLCProperty(meshNode: MeshNode, selectedItemPosition: Int,
                          refreshNodeListener: DeviceListAdapter.DeviceListAdapterListener.RefreshNodeListener) {
        val meshElementControl = getMeshElementControl(meshNode)
        refreshNodeListener.startRefresh()
    ...
    fun setLCProperty(meshNode: MeshNode, selectedItemPosition: Int, data: String) {
        ...
        try {
            ...
            val min = property.characteristic.min
            val max = property.characteristic.max
            listener.showToast(R.string.device_adapter_lc_input_out_of_range, min.toString(), max.toString())
            return
